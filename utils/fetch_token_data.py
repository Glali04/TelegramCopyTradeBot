...
from trade_execution import buy_token
from tracked_tokens import TrackedToken

"""first we will fetch data from dexscreener to get all information about the token, that maybe we will buy. depending on 
how the message was sent in the group, we can get various information about the token, the actual ca or pair id"""


# if market cap is right and token is not on pumpfun or moonshot, we will create an instance of TrackedToken
async def request_token_information(fetch_token_information: dict):
